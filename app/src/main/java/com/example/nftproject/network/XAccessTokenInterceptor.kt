...
class XAccessTokenInterceptor : Interceptor {
	...
	override fun intercept(chain: Interceptor.Chain): Response {
		...
		if (jwtToken != null && response.code == 401) { // 사용하던 토큰이 만료되었다면
			...
			Log.d(TAG, currentToken.toString())
	...
	private fun refreshToken() { // 토큰 재발급
				...
				override fun onResponse(call: okhttp3.Call, response: okhttp3.Response) {
					if(response.isSuccessful){
						...
						if(data.status != 200){ // 토큰 재발급 실패
							...
							clearTokens() // 실패 시 기존의 토큰 삭제하기
						}else{  // 토큰 재발급 성공
							val data = GsonBuilder().create().fromJson(body,
								NewTokenResponse::class.java)

							Log.d("applicationClass", "토큰 재발급 성공")

							if(data.data != null){
								MyApplication.prefUtil.setString(X_ACCESS_TOKEN,
									data.data.accessToken)
								MyApplication.prefUtil.setString(X_REFRESH_TOKEN,
	...
	private fun clearTokens() { // 저장된 액세스 토근과 리프레시 키 삭제하기
